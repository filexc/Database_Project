...
const FilterBar = ({ allItemsData, filters, providers, onFilterChange, onClearFilters }) => {
  ...
  // Update tag2 options when tag1 changes
  useEffect(() => {
    if (filters.tag1 === 'all') {
      setTag2Options([]);
      setShowTag2(false);
    } else {
      const itemsWithTag1 = allItemsData.filter(item => item.tag1 === filters.tag1);
      const secondaryTags = [...new Set(itemsWithTag1.flatMap(item => item.tags))].sort();
      setTag2Options(secondaryTags);
      setShowTag2(secondaryTags.length > 0);
    }
  }, [filters.tag1, allItemsData]);

  // Update tag3 options when tag2 changes
  useEffect(() => {
    if (filters.tag1 === 'all' || filters.tag2 === 'all') {
      ...
      const itemsWithTag1And2 = allItemsData.filter(item => 
        item.tag1 === filters.tag1 && item.tags.includes(filters.tag2)
      );
      const tertiaryTags = [...new Set(
        itemsWithTag1And2.flatMap(item => item.tags).filter(tag => tag !== filters.tag2)
      )].sort();
  ...
  const isAnyFilterActive = filters.tag1 !== 'all' || filters.provider !== 'all' || filters.letter !== null;
  ...
  return (
    <div className="filterBar">
      ...
      {showTag2 && (
        <div className="tagFilters" style={{ display: 'block' }}>
          <select 
            value={filters.tag2} 
